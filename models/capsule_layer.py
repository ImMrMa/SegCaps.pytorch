...
class CapsuleLayer(nn.Module):
    def __init__(self, t_0,z_0, op, k, s, t_1, z_1, routing):
        ...
        for _ in range(t_0):
            if self.op=='conv':
                self.convs.append(nn_.Conv2d(z_0, self.t_1*self.z_1, self.k, self.s))
            else:
                self.convs.append(nn_.ConvTranspose2d(z_0, self.t_1 * self.z_1, self.k, self.s))
...
class ConvLayer(nn.Module):
    def __init__(self):
        ...
        self.conv=nn.Conv2d(16,16,3,1,0)
    def forward(self, x):
        y=self.conv(x)
        return y
